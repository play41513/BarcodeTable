...
{
	if(MessageBoxA(this->Handle,MSGBOX_UNDO.c_str(), NULL, MB_OKCANCEL) == IDOK)
	{
		plMsg->Width = 0;
		//
...
{
	if(MessageBoxA(this->Handle,MSGBOX_BOX_CHANGE.c_str(), NULL, MB_OKCANCEL) == IDOK)
	{
		SetMsgBoxUI(MSG_BOX_CHANGE,clSkyBlue,400);
		plBoxBarcode1->Font->Color = clGray;
		plBoxBarcode2->Font->Color = clGray;
...
//---------------------------------------------------------------------------
void __fastcall TfrmMain::edtBarcodeValueMouseEnter(TObject *Sender)
{
	edtBarcodeValue->SetFocus();
	edtBarcodeValue->SelectAll();
}
